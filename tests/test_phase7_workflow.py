...
def test_combined_export_includes_locked_package_report_and_safety_filters(company_metadata: dict) -> None:
    ...
    assert any(name.endswith("12_Final_Analysis/metrics.csv") for name in names)
    assert any(name.endswith("12_Final_Analysis/citation_audit.json") for name in names)
    assert any(name.endswith("12_Final_Analysis/source_inventory.csv") for name in names)
    assert any(name.endswith("12_Final_Analysis/workflow_audit.json") for name in names)
